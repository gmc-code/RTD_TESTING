...
class GapFillDirective(SphinxDirective):
    ...
    def run(self):
        ...
        while "[" in remaining_text and "]" in remaining_text:
            ...
            # Append plain text leading up to the bracket
            parsed_html_parts.append(html.escape(remaining_text[:start_idx]))

            # Extract the raw bracket contents
            gap_content = remaining_text[start_idx + 1:end_idx].strip()

            # Fallback: If only 1 choice is provided, automatically invent a fake distractor option
            ...
            # Wrap the dropdown and its future validation badge inside an inline container span
            ...
            # Sort alphabetically for initial assembly (JavaScript will shuffle this anyway)
            for opt in sorted(options, key=str.lower):
                dropdown_html += f'<option value="{html.escape(opt.lower())}">{html.escape(opt)}</option>'
            dropdown_html += '</select>'

            # Place an empty badge directly next to the select menu for JS validation injecting
