...
- **Key mismatch in raw lines:** Change ``raw_lines = [item["code"] for item in expected_order]`` to ``raw_lines = [item["code_text"] for item in expected_order]`` so the source list displays correctly.
